...
class AdminWebhooksController extends ModuleAdminController
{
    ...
    /**
     * Display duplicate link
     */
    public function displayDuplicateLink($token, $id, $name = null)
    {
        $tpl = $this->createTemplate('helpers/list/list_action_default.tpl');
        if (!array_key_exists('Duplicate', self::$cache_lang)) {
            self::$cache_lang['Duplicate'] = $this->module->l('Duplicate', 'AdminWebhooks');
        }

        $tpl->assign([
            'href' => $this->context->link->getAdminLink('AdminWebhooks') . '&' . $this->identifier . '=' . $id . '&action=duplicate',
    ...
    public function processDuplicate()
    {
        if ($id = (int) Tools::getValue('id_webhook')) {
            ...
            if (Validate::isLoadedObject($webhook)) {
                $duplicate = new Webhook();
                $duplicate->action = $webhook->action;
                $duplicate->entity = $webhook->entity;
                $duplicate->url = $webhook->url;
                $duplicate->description = $webhook->description . ' (Copy)';
                $duplicate->active = $webhook->active;
                $duplicate->add();

                $this->confirmations[] = $this->module->l('Webhook duplicated successfully.', 'AdminWebhooks');
                return true;
            }
        }

        $this->errors[] = $this->module->l('Webhook could not be duplicated.', 'AdminWebhooks');
        return false;
    }
